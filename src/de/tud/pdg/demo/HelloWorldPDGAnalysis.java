...
/**
 * Main class for a sample forward-branched-flow Hello World PDG generator.
 *
 * @author ifmalcuaz
 * @date 19 Dec 2017
 * @see <a href=
 *      "https://github.com/Sable/soot/wiki/Implementing-an-intra-procedural-data-flow-analysis-in-Soot">Soot
 *      PDG tutorial</a>
 */
public class HelloWorldPDGAnalysis {
    protected Map<Unit, List> unitToGuaranteedDefs;

    public HelloWorldPDGAnalysis(UnitGraph graph) {
        if (Options.v().verbose()) {
            G.v().out.println("[" + graph.getBody().getMethod().getName()
                    + "]     Constructing Sample PDG...");
        }
